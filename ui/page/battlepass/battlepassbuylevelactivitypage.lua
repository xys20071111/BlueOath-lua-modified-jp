...
function BattlePassBuyLevelPage:DoOnOpen()
  ...
  local targetRewardLevelCfg = Logic.battlepassactivityLogic:GetTargetRewardLevelCfg()
...
function BattlePassBuyLevelPage:RegisterAllEvent()
  ...
  UGUIEventListener.AddButtonOnClick(self.tab_Widgets.btnBuy, function()
    ...
    Service.battlepassactivityService:SendBuyPassLevel({
...
function BattlePassBuyLevelPage:Calculate()
  ...
  for lvl = curPassLevel + 1, buyToLevel do
    local cfg = configManager.GetDataById("config_battlepass_level_activity", lvl)
    if curPassType == BATTLEPASSACTIVITY_TYPE.ADVANCED and 0 < cfg.pay_level_reward then
